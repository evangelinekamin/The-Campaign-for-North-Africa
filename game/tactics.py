...
def _cp_ceiling(unit: Unit) -> float:
    """The voluntary-movement CP ceiling (rule 8.16/8.17). A non-motorized unit (CPA of ten
    or less, 8.17) may never voluntarily spend more than 150% of its base CPA in its portion
    of an Operations Stage. A motorized unit (CPA > 10) has NO rule ceiling (8.16 -- it may
    exceed its CPA, paying Disorganization by 6.21); a 2x-CPA soft bound only terminates the
    ...
    return unit.cpa * (1.5 if unit.cpa <= 10 else 2.0)
...
def reachable_for(state: GameState, unit: Unit, enemy_zoc: frozenset,
    ...
    budget = max(0.0, _cp_ceiling(unit) - unit.cp_used)   # 8.17 voluntary ceiling (was hard CPA cap)
