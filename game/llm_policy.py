...
def build_movement_prompt(obs: dict) -> str:
    ...
    if obs.get("victory_cities"):
        ...
        directive = ("FIRST, garrison every victory city with supply_on_hex true -- a stocked "
                     "friendly dump stands ON it, so a combat unit there draws at distance 0 and is "
                     "the cheapest ground you can BANK: those are points you already own, and "
                     "leaving them empty scores nothing. THEN advance only as far as your supply "
                     "follows. A unit whose "
                     "can_hold is false has outrun its supply -- consolidate it back onto a "
        ...
        supply_note = ("A unit that outruns its supply, or a city you take but cannot keep "
                       ...
                       "can_hold is whether a unit HAS IN ITS OWN HEX the week of stores and water "
                       "and the fuel and ammunition 64.73 demands of a holder (distinct from "
                       "ammo-only defensible). CRITICAL: a supply dump can only "
