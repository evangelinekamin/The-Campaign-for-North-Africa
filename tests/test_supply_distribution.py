"""Phase 4, slice S4: the 48 V.C.6 Supply Distribution Segment (engine._supply_distribution) -- the
0-CP Organization-Phase beat that tops a unit's own pools back up from a co-located dump (UNIT_REFILLED,
a conserving dump->unit transfer). FUEL only for now (the 49.14 tank is S1's only seeded contents).

coastal_corridor puts DAK-5le (Fuel Capacity 50) on the same hex as AX-Dump1 (fuel 60); its inline
units start with empty tanks (the tank-fill is oob.build only), which is exactly the drained-tank
fixture this beat is for."""
...
def test_full_tank_draws_nothing():
    # a unit already at capacity has no deficit -> the beat emits nothing for it (why a live run stays
    # byte-identical until S5 begins draining tanks).
    s = coastal_corridor()
    s = s.with_unit(replace(s.unit("DAK-5le"), fuel=50))     # already full
    r = _Run(s)
    _supply_distribution(r, Side.AXIS)
    assert not _dak_refills(r)
    assert r.state.supply("AX-Dump1").fuel == 60            # DAK's dump untouched
