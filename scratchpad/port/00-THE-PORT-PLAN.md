...
| **22.3 — Repair Facilities** | No facility entity exists. **A broken tank sitting in Cairo repairs at the FIELD rate.** The Temporary/Major columns (50/33/25/25/10/10 and 75/50/50/50/33/33/25/10) and the die modifiers **are transcribed in `data/breakdown_rates.json` and nothing reads them.** | **HIGH — this is the Commonwealth's entire rear-area recovery economy.** | **L** | **DONE (2026-07-30/31, `game/repair.py`)** — the MAJOR-facility half (Alexandria/Cairo/Tobruk, "already in existence" per 24.81, no construction needed). 24.8 Temporary Facility construction stays deferred (no village/town hex roster to site one on, a chart-vs-prose Fuel/Op-Stage contradiction, no Policy would ever order one) — see `game/repair.py`'s module docstring and `scratchpad/port/transcriptions/22.3-cw-rear-area-recovery.md`. |
...
| **56.3 — Axis coastal shipping** | **DONE (2026-07-31, `game/engine.py::_coastal_shipping` + `game/state.py::CoastalShip`)** — the 4-ship fleet (three 1,000t hulls, one 2,000t, transcribed off the vmod counters, no scan roster exists), CPA-50/load-5/unload-5, single-commodity, immune to attack, gated on 56.33 neutralized/enemy ports. Seeded at `PORT-Benghazi` (Tripoli has no hex in this engine; Benghazi already plays Tripoli's engine role). Doctrine (`campaign_policy.coastal_shipping_doctrine`, flagged an opinion): a Benghazi\<-\>Tobruk relief shuttle. **MEASURED, 111-turn campaign, 5 seeds: gross tons moved is large (530k-620k) but mostly a ping-pong wash — NET forward delivery is modest (4k-119k tons of Fuel, seed-dependent) and, decisively, an A/B (ships on vs off, byte-identical seed) shows truck last-mile tonnage UNCHANGED to the ton in all 3 seeds tested. 56.3 does NOT relieve the truck bottleneck — it is a pure ADDITION, running on its own supply, never substituting for the lorry park. Winner/VP unchanged in every seed.** See `scratchpad/port/transcriptions/56.3-axis-inter-port-transport.md`. | ~~M~~ | ~~60.35 seeds them all in Tripoli.~~ |
